...
Raw API example (PowerShell):

```powershell
$body = @{ message = "Where is my order ORD-1001?"; owner_id = "shopper:alice@example.com" } | ConvertTo-Json
Invoke-RestMethod http://localhost:8000/chat -Method Post -ContentType "application/json" -Body $body | ConvertTo-Json -Depth 6
```

---

## 3. Cheat sheet — what's actually in the database
...
## 4. Scenario A — one prompt per agent (routing precision)
...
## 5. Scenario B — multi-agent turns (one-or-more routing)
...
|---|---|---|---|
| B1 | *Where is order ORD-1003, and can I still return it if the sweater doesn't fit?* | `order_tracking` + `returns_refunds` | one coherent reply covering status (processing, ETA 2026-07-02) **and** return policy/eligibility |
...
## 6. Scenario C — deliberate ambiguity (the interesting cases)
...
## 7. Scenario D — peer delegation (`ask_<agent>` tools)
...
| D2 | *I want to return ORD-1002 — but first, has it actually been delivered?* | `returns_refunds` may call `ask_order_tracking` (delivered 2026-06-18 → then RMA details). |

## 8. Scenario E — memory across sessions

Memory is scoped by `owner_id` (set it in the field at the top of the Chat page, e.g. `shopper:alice@example.com`, and keep it the same).

1. **Chat 1:** *"By the way — I'm Alice, I always wear size M and I only buy natural fabrics, no polyester."* (any reply is fine)
2. **Start a new chat** (new conversation, same `owner_id`): *"Recommend a top for me."*
   - **Pass:** the recommendation reflects size M and/or natural fabrics without you repeating them.
3. Check **Dashboard → memory stats** (or `GET /analytics/memory?owner_id=shopper:alice@example.com`) — stored memories should be > 0.
4. Force a consolidation run (Layer 4 "dreaming") and check it reports merged/pruned counts:
   ```powershell
   Invoke-RestMethod "http://localhost:8000/dream?owner_id=shopper:alice@example.com&force=true" -Method Post
   ```
5. **Control test:** repeat step 2 with the `owner_id` field **empty** — the assistant should NOT know your size (no memory without an owner).

## 9. Scenario F — conversation continuity
...
## 10. Scenario G — evaluation & feedback
...
## 11. Scenario H — fail-silent & feature flags
...
- [ ] E: preferences recalled in a new conversation with the same `owner_id`; not recalled without one; `/dream` runs
...
- **`401` / provider errors in agent replies:** check `.env` has `OPENAI_API_KEY` and `DEFAULT_PROVIDER=openai`.
