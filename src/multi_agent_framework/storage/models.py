...
class Conversation(Base):
    ...
    total_cost: Mapped[Decimal] = mapped_column(Numeric(12, 6), nullable=False, server_default="0")
    compaction_count: Mapped[int] = mapped_column(Integer, nullable=False, server_default="0")

    turns: Mapped[list["ConversationTurn"]] = relationship(back_populates="conversation", cascade="all, delete-orphan", passive_deletes=True)
...
class AgentPerformance(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint("agent_name", "query_category", name="pk_agent_performance"),)
